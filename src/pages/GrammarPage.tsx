...
const GrammarPage = () => {
  ...
  useEffect(() => {
    const initializeApiKey = async () => {
      try {
        ...
        if (envApiKey && envApiKey.trim() !== '') {
          ...
          if (savedApiKey && savedApiKey.trim() !== '') {
            try {
              ...
              console.warn('[GrammarPage] Invalid API key in localStorage:', error);
        ...
        console.error('[GrammarPage] Failed to initialize API key:', error);
        // Don't navigate away, let user enter API key manually
      }
  ...
  const handleApiKeySubmit = async (key: string) => {
    try {
      ...
      console.error('[GrammarPage] Failed to set API key:', error);
      ...
      <div className="mb-6 sm:mb-8 text-center">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">
          <span className="flex items-center justify-center gap-2">
            <BookOpen className="text-kid-green w-6 h-6 sm:w-8 sm:h-8" />
            <span>English Grammar Learner</span>
          </span>
        </h1>
      </div>
