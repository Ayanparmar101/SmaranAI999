...
} from '@/components/social-science/socialScienceUtils';

const SocialScienceLearnerPage = () => {
  const navigate = useNavigate();
  const [apiKey, setApiKey] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [selectedLevel, setSelectedLevel] = useState<'beginner' | 'intermediate' | 'advanced'>('beginner');
  const [selectedTopic, setSelectedTopic] = useState<string>('');
  const [lesson, setLesson] = useState<SocialScienceLesson | null>(null);
  ...
  useEffect(() => {
    const initializeApiKey = async () => {
      try {
        ...
        if (envApiKey && envApiKey.trim() !== '') {
          ...
          if (savedApiKey && savedApiKey.trim() !== '') {
            try {
              ...
              console.warn('[SocialScienceLearnerPage] Invalid API key in localStorage:', error);
        ...
        console.error('[SocialScienceLearnerPage] Failed to initialize API key:', error);
      }
  ...
  const handleApiKeySubmit = async (key: string) => {
    try {
      ...
      console.error('[SocialScienceLearnerPage] Failed to set API key:', error);
  ...
  const generateLesson = async (topic: string) => {
    ...
    try {
      ...
      const systemPrompt = `You are an expert Social Science teacher for elementary school students. Create an engaging Social Science lesson about "${topic}" for ${promptLevel}. The lesson difficulty should be "${difficultyLevel}".
      
      Your response must follow this exact JSON format without any markdown formatting or extra text:
      {
        "title": "Lesson title",
        "content": "A clear, simple explanation of the social science concept with cultural context (keep it concise)",
      ...
      2. Keep the explanation and content brief and simple with interesting social facts.
      ...
      5. Include social concepts, geography, history, and civics appropriate for the grade level.`;

      const result = await openaiService.createCompletion(systemPrompt, 'Generate a social science lesson', {
        max_tokens: 3000,
        temperature: 0.7
      });
  ...
  return (
    <div className="page-container">
      <div className="mb-6 sm:mb-8 text-center">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">
          <span className="flex items-center justify-center gap-2">
            <Globe className="text-kid-orange w-6 h-6 sm:w-8 sm:h-8" />
            <span>Social Science Learner</span>
          </span>
        </h1>
      </div>
      ...
      <TopicSelector 
        topics={socialScienceTopics[selectedLevel]} 
...
export default SocialScienceLearnerPage;
