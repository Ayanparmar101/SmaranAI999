...
import { saveMessage } from '@/utils/messageUtils'; // Use Firebase message utils
...
const StudyPlannerContainer = () => {
  ...
  // handleFileUpload is no longer returned by useChapterContent
  ...
  const handleChapterSelect = (chapterId: string) => {
    console.log('[StudyPlannerContainer] Chapter selected via dropdown:', chapterId);
    setSelectedChapter(chapterId);
  };

  const handleResetPDF = () => {
    console.log('[StudyPlannerContainer] Resetting PDF and selected chapter.');
    setSelectedChapter(""); 
  };
  ...
  const verifyApiKey = async (): Promise<boolean> => {
    try {
      ...
      console.error('[StudyPlannerContainer] Failed to verify API key:', error);
  ...
  const handleGenerateStudyPlan = async () => {
    console.log('[StudyPlannerContainer] handleGenerateStudyPlan called for chapter:', selectedChapter, 'book:', selectedBook);
  ...
  const saveStudyPlan = async () => {
    ...
    console.log('[StudyPlannerContainer] Saving study plan for user:', user.uid);
    try {
      const success = await saveMessage({
        text: `${selectedBook} - ${selectedChapter}`,
        userId: user.uid,
        aiResponse: JSON.stringify(studyPlan),
        chatType: 'study-planner' as any, // Cast to handle the type constraint
        additionalData: {
    ...
    <div className="container mx-auto py-6 px-4 max-w-4xl">
      ...
      <Tabs defaultValue="create" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="create">Create Study Plan</TabsTrigger>
