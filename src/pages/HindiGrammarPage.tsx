...
const hindiGrammarTopics = {
  beginner: [
    'वर्ण और मात्राएं (Letters and Vowel Signs)',
    'संज्ञा (Nouns)',
    'सर्वनाम (Pronouns)', 
    'विशेषण (Adjectives)',
    'वचन (Singular/Plural)',
  ...
  advanced: [
    ...
    'रस (Emotions in Poetry)',
    'व्याकरण के नियम (Grammar Rules)',
    'मुहावरे (Idioms)',
    'लोकोक्तियां (Proverbs)',
  ],
};

const HindiGrammarPage = () => {
        ...
        if (envApiKey && envApiKey.trim() !== '') {
          ...
          if (savedApiKey && savedApiKey.trim() !== '') {
            try {
              ...
              console.warn('[HindiGrammarPage] Invalid API key in localStorage:', error);
              localStorage.removeItem('openaiApiKey');
              // Don't navigate away, let user enter API key manually
              return;
            }
          }
          // Don't navigate away if no API key, let the component handle it
        }
      } catch (error) {
        console.error('[HindiGrammarPage] Failed to initialize API key:', error);
        // Don't navigate away, let user enter API key manually
      }
  ...
  const handleApiKeySubmit = async (key: string) => {
    try {
      ...
      console.error('[HindiGrammarPage] Failed to set API key:', error);
  ...
  const generateLesson = async (topic: string) => {
    ...
    try {
      ...
      const systemPrompt = `You are an expert Hindi teacher for elementary school students. Create an engaging Hindi grammar lesson about "${topic}" for ${promptLevel}. The lesson difficulty should be "${difficultyLevel}".
      
      Your response must follow this exact JSON format without any markdown formatting or extra text:
      {
        "title": "Lesson title in Hindi",
        "content": "A clear, simple explanation of the grammar concept in Hindi (keep it concise)",
        "level": "${difficultyLevel}",
        "examples": ["Example 1 in Hindi", "Example 2 in Hindi", "Example 3 in Hindi"],
        "quiz": {
          "${selectedDifficulty}": [
            {
              ...
              "correctIndex": 0,
              "explanations": ["Explanation for A in Hindi", "Explanation for B in Hindi", "Explanation for C in Hindi", "Explanation for D in Hindi"]
            }
          ]
        }
      }
      ...
      2. Keep the explanation and content brief and simple in Hindi.
      ...
      5. Use proper Hindi grammar and Devanagari script.`;

      const result = await openaiService.createCompletion(systemPrompt, 'Generate a Hindi grammar lesson', {
        max_tokens: 3000,
        temperature: 0.7
      });
    ...
    <div className="page-container">
      <div className="mb-6 sm:mb-8 text-center">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">
          <span className="flex items-center justify-center gap-2">
            <BookOpen className="text-kid-red w-6 h-6 sm:w-8 sm:h-8" />
            <span>हिंदी व्याकरण सीखें (Hindi Grammar Learner)</span>
          </span>
        </h1>
      </div>
      ...
      <div className="mb-8 sm:mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 max-w-4xl mx-auto">
          {hindiGrammarTopics[selectedLevel].map((topic) => (
            <button
              key={topic}
              ...
              className={`p-3 sm:p-4 border-3 border-black rounded-xl transition-all min-h-[48px] sm:min-h-[44px] text-sm sm:text-base font-medium text-center ${
                selectedTopic === topic
                  ? 'bg-kid-red text-white shadow-none translate-y-1'
...
export default HindiGrammarPage;
