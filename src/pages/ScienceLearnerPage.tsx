...
} from '@/components/science/scienceUtils';

const ScienceLearnerPage = () => {
  const navigate = useNavigate();
  const [apiKey, setApiKey] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [selectedLevel, setSelectedLevel] = useState<'beginner' | 'intermediate' | 'advanced'>('beginner');
  const [selectedTopic, setSelectedTopic] = useState<string>('');
  const [lesson, setLesson] = useState<ScienceLesson | null>(null);
  ...
  useEffect(() => {
    const initializeApiKey = async () => {
      try {
        ...
        if (envApiKey && envApiKey.trim() !== '') {
          ...
          if (savedApiKey && savedApiKey.trim() !== '') {
            try {
              ...
              console.warn('[ScienceLearnerPage] Invalid API key in localStorage:', error);
        ...
        console.error('[ScienceLearnerPage] Failed to initialize API key:', error);
      }
  ...
  const handleApiKeySubmit = async (key: string) => {
    try {
      ...
      console.error('[ScienceLearnerPage] Failed to set API key:', error);
  ...
  const generateLesson = async (topic: string) => {
    ...
    try {
      ...
      const systemPrompt = `You are an expert Science teacher for elementary school students. Create an engaging Science lesson about "${topic}" for ${promptLevel}. The lesson difficulty should be "${difficultyLevel}".
      
      Your response must follow this exact JSON format without any markdown formatting or extra text:
      {
        "title": "Lesson title",
        "content": "A clear, simple explanation of the science concept with interesting facts (keep it concise)",
      ...
      2. Keep the explanation and content brief and simple with fascinating science facts.
      ...
      5. Include scientific concepts and examples appropriate for the grade level.`;

      const result = await openaiService.createCompletion(systemPrompt, 'Generate a science lesson', {
        max_tokens: 3000,
        temperature: 0.7
      });
  ...
  return (
    <div className="page-container">
      <div className="mb-6 sm:mb-8 text-center">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">
          <span className="flex items-center justify-center gap-2">
            <Microscope className="text-kid-purple w-6 h-6 sm:w-8 sm:h-8" />
            <span>Science Learner</span>
          </span>
        </h1>
      </div>
      ...
      <TopicSelector 
        topics={scienceTopics[selectedLevel]} 
...
export default ScienceLearnerPage;
